...
def market_buy(amount, symbol):
    ret = send_order(amount, "", symbol, "buy-market")
    times = 1
    while not success(ret):
        print "Buy %s %s failed %s times" % (symbol, amount, times)
        ret = send_order(amount, "", symbol, "buy-market")
        times += 1
    print "Buy %s %s success %s times" % (symbol, amount, times)


if __name__ == '__main__':
    # market_buy(10, "btcusdt")
